...
import { Check, X, Trash2, Clock, CheckCircle2, XCircle } from "lucide-react";
...
export default function AdminInvestments() {
  ...
  const setStatus = async (r: Row, status: "approved" | "rejected", extraNote?: string) => {
    ...
    if (status === "approved" && r.profile) {
      // Debit user balance to fund the investment
      const newBal = Number(r.profile.balance) - Number(r.amount);
      await supabase.from("profiles").update({ balance: newBal }).eq("id", r.profile_id);
      await supabase.from("transactions").insert({
        profile_id: r.profile_id,
        type: "debit",
        amount: r.amount,
        description: `Investment approved: ${r.asset.replace("_", " ")}`,
    ...
    <DashboardLayout>
      <div className="flex items-center justify-between mb-6 gap-3 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Review and approve user investments</p>
        </div>
